...
def mtm_svd_lfv(ts2d,psi,kk,dt) :
	...
	vmrep = repmat(vm,ts2d.shape[0],1) # Repeat the mean array to match the shape of ts2d
	ts2d = ts2d - vmrep # Subtract the mean from ts2d
	vs = np.nanstd(ts2d, axis=0) # Calculate the standard deviation along the time axis
	vsrep = repmat(vs,ts2d.shape[0],1) # Repeat the standard deviation array to match the shape of ts2d
	ts2d = np.divide(ts2d,vsrep) # Divide ts2d by the standard deviation
	...
	psimats = []
	for k in range(kk):
		psimat2 = np.transpose(repmat(psi[k,:],ts2d.shape[1],1) ) # Repeat each taper to match the shape of ts2d
		psimat2 = np.multiply(psimat2,ts2d) # Multiply each taper with ts2d element-wise
		psimats.append(psimat2) # Append the resulting matrix to the list
	psimats=np.array(psimats) # Convert the list to a numpy array
...
def monte_carlo_test(index_with_space, niter, sl, len_freq, psi, kk, dt):
	...
	- niter: Number of iterations for the Monte Carlo test.
	- sl: Significance level for the confidence interval.
	...
	for ii in range(niter):
		...
		[freq_rd, lfv_rd] = mtm_svd_lfv(index_with_space, psi, kk, dt)  # calculate the LFV using the shuffled index_with_space
...
def mtm_svd_recon(ts2d, psi, kk, dt, fo) :
	...
	lan = 0  # Initialize the lan variable
	vw = 0  # Initialize the vw variable
	...
	# Get the matrix of spectrums
	psimats = []  # Initialize an empty list to store the matrices of spectrums
	for k in range(kk):
		psimat2 = np.transpose(repmat(psi[k,:],ts2d.shape[1],1) )  # Repeat each taper to match the shape of ts2d
		psimat2 = np.multiply(psimat2,ts2d)  # Multiply each taper with ts2d element-wise
		psimats.append(psimat2)  # Append the resulting matrix to the list
	...
	D = vsrep  # Assign the standard deviation array to D

	envmax = np.zeros(len(fo))*np.nan  # Initialize an array of zeros with length equal to the number of frequencies of interest
	...
	for i1 in range(len(fo)):
		...
		cs=[1]  # Initialize an array with the value 1
		sn=[0]  # Initialize an array with the value 0
		c=np.cos(2*np.pi*ff0*dt)  # Calculate the cosine term
		s=np.sin(2*np.pi*ff0*dt)  # Calculate the sine term
		for i2 in range(1,p):
			cs.append( cs[i2-1]*c-sn[i2-1]*s )  # Calculate the cosine term
			sn.append( cs[i2-1]*s+sn[i2-1]*c )  # Calculate the sine term
		CS = [complex(cs[i], sn[i]) for i in range(len(cs))]  # Calculate the complex number
